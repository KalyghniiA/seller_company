package ru.kalyghnii.pet.seller_company.util;

public class Constant {
    public final static String MAPPER = "object-mapper";
    public final static String PERSON_SERVICE = "person-service";
    public final static String PERSON_ELEMENT = "person-element";
    public final static String RESPONSE_ELEMENT = "response-element";
    public final static String REQUEST_PARAM_KEY_ID = "id";
    public final static String REQUEST_PARAM_VALUE_ID = "id-value";


    private Constant(){}
}
